...
for numero_colonne,nom_feuille in enumerate(fichier.sheetnames,start=0):
    ...
    if nom_feuille=='suivre_chemin':
        liste_coef=np.polyfit(liste_score,liste_pas_moyen,2)
        print(liste_coef)

        liste_y=(lambda t:sum([liste_coef[len(liste_coef)-1-i]*t**i for i in range(len(liste_coef))]))(np.array(liste_score))

        axe_primaire2.plot(liste_score,liste_y,color=couleurs[numero_colonne],linestyle=':')
