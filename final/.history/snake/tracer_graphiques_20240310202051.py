...
for numero_colonne,nom_feuille in enumerate(fichier.sheetnames,start=0):
    ...
    liste_score_tronquee_derivee=100*np.diff(liste_score_tronquee)/np.diff(liste_nb_pas_total)
    # plot du score en fonction du nb de pas total
    axe_primaire2.plot(liste_nb_pas_total[:-1],liste_score_tronquee_derivee,color=couleurs[numero_colonne],label=nom_feuille,linestyle='--')
