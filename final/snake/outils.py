        ...
        for y in range(nb_cases):
            ...
            for voisin in liste_voisins:
                if not ((str(voisin[0]),str(voisin[1])),(str(x),str(y))) in deja_traites:    # inutile
                    ...
                    graphe.add_node((f"{x},{voisin[0]}\'",f"{y},{voisin[1]}\'"))  # x'
                    graphe.add_node((f"{voisin[0]},{x}\'",f"{voisin[1]},{y}\'"))  # x'
                    
                    # (x,x')
                    graphe.add_edge(
                        (f"{x}",f"{y}"),
                        (f"{x},{voisin[0]}\'",f"{y},{voisin[1]}\'")
                        )
                    
                    # (y,y')
                    graphe.add_edge(
                        (f"{voisin[0]}",f"{voisin[1]}"),
                        (f"{voisin[0]},{x}\'",f"{voisin[1]},{y}\'")
                        )
            ...
            deja_traites.add(((str(x),str(y)),(str(voisin[0]),str(voisin[1]))))
...
def avoir_couplage_parfait(graphe_,liste_sommets_a_enlever=None):
    ...
    # couplage=set([list(graphe.edges)[0]])
    ...
    while not nx.is_perfect_matching(graphe,couplage):
        ...
        for sommet in graphe.nodes():
            ...
            if sommet not in liste_sommets_couplage:
                ...
                for sommet_voisin in graphe.neighbors(sommet):
                    ...
                    if sommet!=sommet_depart:
                        if sommet_voisin not in deja_traites and sommet_voisin not in a_traiter:
                            parent_sommet=dico_parents[sommet]

                            if (parent_sommet,sommet) in couplage or (sommet,parent_sommet) in couplage:
        ...
        # chemin
        chemin=chemin_depuis_sommet(dico_parents,sommet_final,sommet_depart)


        # print(chemin)


        nouveau_couplage=set()

        for i in range(0,len(chemin)-1,2):
            nouveau_couplage.add((chemin[i],chemin[i+1]))
        
        for couple in couplage:
            v1,v2=couple
            if not v1 in chemin and not v2 in chemin:
                nouveau_couplage.add(couple)
...
def chemin_depuis_sommet(dico_parents,sommet,sommet_depart):
    chemin=[]
...
def deuxfacteur(nb_cases,couplage):
    ...
    for couple in couplage:
        ...
        aretes_interdites.append(((int(n11[:i_n11]),int(n12[:i_n12])),(int(n21[:i_n21]),int(n22[:i_n22]))))
